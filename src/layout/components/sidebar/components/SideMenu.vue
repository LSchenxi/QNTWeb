...
import { renderIcon, isExternal, storage } from '@/utils'
...
const menuOptions = computed(() => {
  ...
  let userType = storage.get(account)
  let menus = permissionStore.menus
  menus.forEach(
    (menuOne) => {
      if(menuOne.name === 'userManagement'){
        index = menus.indexOf(menuOne)
      }
    }
  )
  if(userType!==0) {
    let newMenus = menus.splice(index, 1)
    newMenus = menus.filter(item => item !== newMenus )
  }
